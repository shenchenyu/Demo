...
public class PullListView extends ListView implements OnScrollListener {
	...
	private PListViewBottom mBottom;
	...
	private void init(Context context) {
		...
		mBottom = new PListViewBottom(context);
	...
	public void setPullLoadEnable(boolean enable){
		...
		if(mEnablePullLoad){
			mBottom.show();
			mBottom.setState(PListViewBottom.STATE_NORMAL);
			//设置分割线
			...
			mBottom.hide();
			mBottom.setOnClickListener(null);
	...
	protected void startLoadingMore() {
		mPullLoading = true;
		mBottom.setState(PListViewBottom.STATE_REFRESHING);
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE://移动动作
			...
			if(getFirstVisiblePosition()==0&&(mHeaderView.getVisiableHeight()>0||dy>0)){//如果第一个可见的话
				...
				invokeOnScrolling();
			}else if(getLastVisiblePosition()==(mTotalCount-1)&&(mBottom.getBottom()>0||dy<0)){
